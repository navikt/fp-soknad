...
public class FrontendTypegenereringOpenApiConfig extends OpenApiContextBuilder{
    private static final Set<Class<?>> INKLUDER_CLASSER = ApiConfig.getApplicationClasses();

    private FrontendTypegenereringOpenApiConfig() {
        // Hide
    }

    public static void registerOpenApi(Application application) {
        var oasConfig = buildOpenApiConfig("Fpsoknad - specifikasjon for typegenerering frontend", application)
            .readerClass(FrontendTypegenereringOpenApiConfig.TypegenereringFrontendOpenApiReader.class.getName());
     ...
     *  1) Filtrere bort klasser som ikke er angitt
     *  2) Her fjerner vi path (vi ønsker ikke mulighet til å kalle endepunktene via swagger)
     *  3) Registere ModulConverter med egendefinert Objectmapper i henhold til hva frontend forventer
     */
    public static class TypegenereringFrontendOpenApiReader extends Reader {

        @Override
        public OpenAPI read(Set<Class<?>> resourceClasses) {
            lagCustomModelConvertersForFrontendTypegenerering();
            var openApi = super.read(resourceClasses.stream()
