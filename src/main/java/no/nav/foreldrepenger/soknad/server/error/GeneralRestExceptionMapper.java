...
import no.nav.foreldrepenger.soknad.innsending.error.ForsendelseIkkeFunnetException;
...
public class GeneralRestExceptionMapper implements ExceptionMapper<Throwable> {
    ...
    public Response toResponse(Throwable exception) {
        ...
        if (exception instanceof ForsendelseIkkeFunnetException e) {
            LOG.info(e.getMessage());
            return status(HttpStatus.GONE_410, FeilKode.FORSENDELSE_FINNES_IKKE, e.getMessage());
        }
